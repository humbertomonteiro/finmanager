...
export const CreditSalesList: React.FC = () => {
  ...
  const totalPending = useMemo(() => pending.reduce((s, t) => s + t.value, 0), [pending]);
  const totalPaid    = useMemo(() => paid.reduce((s, t) => s + t.value, 0),    [paid]);

  const displayed = useMemo(() => {
    let list = activeTab === "pending" ? pending : activeTab === "paid" ? paid : credits;
    if (search.trim()) {
      list = list.filter((t) =>
        (t.customerName || "").toLowerCase().includes(search.toLowerCase())
      );
    }
    return [...list].sort((a, b) => new Date(b.date as Date).getTime() - new Date(a.date as Date).getTime());
  }, [activeTab, pending, paid, credits, search]);

  const handleMarkPaid = async (t: Transaction) => {
    if (!confirm(`Confirmar recebimento de ${formatCurrency(t.value)} de ${t.customerName}?`)) return;
  ...
  const byCustomer = useMemo(() => {
    return pending.reduce<Record<string, { total: number; count: number }>>((acc, t) => {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.summaryGrid}>
        <div className={`${styles.summaryCard} ${styles.summaryPending}`}>
          ...
          <div className={styles.summaryAmount}>{formatCurrency(totalPending)}</div>
        </div>
        <div className={`${styles.summaryCard} ${styles.summaryPaid}`}>
          ...
          <div className={styles.summaryAmount}>{formatCurrency(totalPaid)}</div>
        </div>
        <div className={styles.summaryCard}>
          <div className={styles.summaryLabel}>Total geral</div>
          <div className={styles.summaryCount}>{credits.length}</div>
          <div className={styles.summaryAmount}>{formatCurrency(totalPending + totalPaid)}</div>
        </div>
      </div>
      ...
      <div className={styles.controls}>
        <div className={styles.tabs}>
          {(["pending", "paid", "all"] as CreditTab[]).map((tab) => (
            ...
            >
              {tab === "pending" ? `⏳ Pendentes (${pending.length})` : tab === "paid" ? `✓ Pagas (${paid.length})` : `Todas (${credits.length})`}
            </button>
          ))}
        </div>
      ...
      {displayed.length === 0 ? (
        <div className={styles.empty}>
          ...
          <div className={styles.emptyTitle}>
            {activeTab === "pending" ? "Nenhuma venda pendente" : "Nenhuma venda encontrada"}
          </div>
        ...
        <div className={styles.list}>
          {displayed.map((sale) => (
            <div
              key={sale.id}
              className={`${styles.card} ${sale.isPaid ? styles.cardPaid : styles.cardPending}`}
            >
              <div className={styles.cardHeader}>
                <div className={styles.customerInfo}>
                  ...
                  <div>
                    <div className={styles.customerName}>{sale.customerName || "Sem nome"}</div>
